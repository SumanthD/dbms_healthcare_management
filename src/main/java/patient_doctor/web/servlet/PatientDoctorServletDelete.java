...
public class PatientDoctorServletDelete extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public PatientDoctorServletDelete() {
        super();
    }
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(method.equals("search"))
		{
			...
			if(patient_doctor.getPatient_id()!=null && patient_doctor.getDoctor_id()!=null){
						...
						request.getRequestDispatcher("/jsps/patient_doctor_entity/patient_doctor_delete_output.jsp").forward(request, response);			
				}
				else{
		...
		{	
			try {
				patient_doctorDao.delete(Integer.parseInt(request.getParameter("patient_id")), Integer.parseInt(request.getParameter("doctor_id")));
			} catch (ClassNotFoundException e1) {
				e1.printStackTrace();
			} catch (InstantiationException e1) {
				e1.printStackTrace();
			...
			request.setAttribute("msg", "PatientDoctor Deleted");
			request.getRequestDispatcher("/jsps/patient_doctor_entity/patient_doctor_read_output.jsp").forward(request, response);
		}
	}
}
